...
module.exports = {
	...
	tests: [
		...
		function testChainReactionRespectsEnabled(test) {
			// Exercise success and failure chains without launching real jobs or
			// changing shared storage.  Both paths use the same chainReaction method.
			var chain_reaction = require('./scheduler.js').prototype.chainReaction;
			
			[0, 1].forEach( function(code) {
				[0, 1].forEach( function(enabled) {
					var event = {
						id: 'chain_target', title: 'Chain Target', enabled: enabled,
						queue: 1, params: { original: 'value', override: 'before' }
					};
					var old_job = {
						id: 'chain_source_job', event: 'chain_source', event_title: 'Chain Source',
						code: code, description: code ? 'Test failure' : '',
						chain_data: { value: 42 }, chain_params: { override: 'after' },
						notify_fail: 'test@localhost'
					};
					var captured_job = null;
					
					// Supply only the collaborators needed by the real method.  A
					// disabled target should skip silently, without failure notifications.
					var context = {
						storage: {
							listFind: function(key, criteria, callback) {
								test.ok( key == 'global/schedule', "Chain target is read from current schedule" );
								test.ok( criteria.id == event.id, "Correct chain target is looked up" );
								callback(null, event);
							}
						},
						logDebug: function() {},
						logError: function() { test.ok(false, "Chain target should not generate an error"); },
						logActivity: function() { test.ok(false, "Chain target should not generate a warning"); },
						sendEventErrorEmail: function() { test.ok(false, "Chain target should not send a failure email"); },
						launchOrQueueJob: function(job, callback) {
							captured_job = job;
							callback(null, []);
						}
					};
					
					chain_reaction.call( context, old_job, event.id );
					test.ok( !!captured_job == !!enabled, "Only enabled chain targets are launched or queued (code " + code + ")" );
					
					// Enabled targets retain the original chain context and parameter merging.
					if (enabled) {
						test.ok( captured_job.source_event == old_job.event, "Source event is preserved" );
						test.ok( captured_job.source_job == old_job.id, "Source job is preserved" );
						test.ok( captured_job.chain_code == code, "Source exit code is preserved" );
						test.ok( captured_job.chain_description == old_job.description, "Source description is preserved" );
						test.ok( captured_job.chain_data.value == 42, "Chain data is preserved" );
						test.ok( captured_job.params.original == 'value', "Default target params are preserved" );
						test.ok( captured_job.params.override == 'after', "Chain params override target params" );
						test.ok( event.params.override == 'before', "Stored target params are unchanged" );
					}
				} );
			} );
