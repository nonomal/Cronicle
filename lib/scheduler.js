...
module.exports = Class.create({
	...
	chainReaction: function(old_job, chain_event_id) {
		...
		this.storage.listFind( 'global/schedule', { id: chain_event_id }, function(err, event) {
			...
			// Respect the target's current enabled state for both success and error chains.
			// Skip before launchOrQueueJob so disabled events cannot be queued either.
			if (!event.enabled) {
				self.logDebug(6, "Skipping chain reaction: Event is disabled: " + event.id + " (" + event.title + ")");
				return;
			}
